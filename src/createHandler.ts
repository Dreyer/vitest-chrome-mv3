...
export function createHandler(
  schema: ChromeSchema | NamespaceSchema = vitestChromeSchema as ChromeSchema,
  namespace = 'chrome',
  ...
  const handler: ProxyHandler<VitestChrome> & {
    ...
    get(target, key) {
      ...
      if (!schema) {
        ...
        if (
          keyStr.startsWith('on') &&
          keyStr.length > 2 &&
          keyStr[2] === keyStr[2].toUpperCase()
        ) {
          // Event
          const event = addEvent(
            { name: keyStr, type: 'event', parameters: [] },
            target,
          );
          ...
          (callableFn as MockWithClear<unknown[], Promise<never>>).clear =
            () => {
          ...
          const combinedProxy = new Proxy(targetObj, {
            ...
            ownKeys() {
              return proxyHandler.ownKeys
                ? proxyHandler.ownKeys(targetObj)
                : [];
            },
            ...
            has(_target, key) {
              return proxyHandler.has
                ? proxyHandler.has(targetObj, key)
                : false;
            },
            set(_target, key, value, receiver) {
              return proxyHandler.set
                ? proxyHandler.set(targetObj, key, value, receiver)
                : true;
            },
      ...
      if (schema && key in schema) {
        let mockedElement: unknown;
        switch ((schema[key as keyof typeof schema] as SchemaMember).type) {
          case 'event':
            mockedElement = addEvent(
              schema[key as keyof typeof schema] as SchemaMember,
              target,
            );
            break;
          case 'function':
            mockedElement = addFunction(
              schema[key as keyof typeof schema] as SchemaMember,
              target,
              namespace,
            );
          ...
          default: {
            ...
            if (key === 'storage' && namespace === 'chrome') {
              ...
              const onChangedEvent = addEvent(
                { name: 'onChanged', type: 'event', parameters: [] },
                {},
              );
              ...
              if (
                typeof (localArea as { clearAll?: () => void }).clearAll ===
                'function'
              )
                mocks.add((localArea as { clearAll: () => void }).clearAll);
              if (
                typeof (syncArea as { clearAll?: () => void }).clearAll ===
                'function'
              )
                mocks.add((syncArea as { clearAll: () => void }).clearAll);
              if (
                typeof (managedArea as { clearAll?: () => void }).clearAll ===
                'function'
              )
                mocks.add((managedArea as { clearAll: () => void }).clearAll);
              if (typeof onChangedEvent.clear === 'function')
                mocks.add(onChangedEvent.clear);
              ...
              const proxy = new Proxy<Record<string, unknown>>(
                {},
                createHandler(
                  schema[key as keyof typeof schema] as NamespaceSchema,
        ...
        if (
          mockedElement &&
          typeof (mockedElement as { clear?: () => void }).clear === 'function'
        ) {
          mocks.add((mockedElement as { clear: () => void }).clear);
        }
        return mockedElement;
      } else {
        // Handle case where schema exists and has functions/events arrays
        if (schema && Array.isArray((schema as NamespaceSchema).functions)) {
          const functionSchema = (schema as NamespaceSchema).functions.find(
            (fn) => fn.name === key,
          );
          if (functionSchema) {
            const fn = addFunction(functionSchema, target, namespace);
            Object.assign(target, { [key]: fn });
            if (typeof (fn as MockWithClear).clear === 'function') {
              mocks.add((fn as MockWithClear).clear);
            }
            return fn;
          }
        }

        if (schema && Array.isArray((schema as NamespaceSchema).events)) {
          const eventSchema = (schema as NamespaceSchema).events.find(
        ...
        if (
          ...
          const event = addEvent({ name: keyStr, type: 'event' }, target);
          if (typeof event.clear === 'function') {
            mocks.add(event.clear);
          }
          return event;
