...
- Key scripts:
  ...
  - `npm run build:mock` – generate `src/generated.ts` from the schema (runs automatically during `prepublishOnly`).

## Repository Conventions
- TypeScript source (`src/`) plus generated schema `src/vitest-chrome-schema.json` and generated mock `src/generated.ts`.
- `src/vitest-chrome-schema.json` is a generated intermediate artifact that represents the structured Chrome API surface (namespaces, functions, events, properties). It serves as the source of truth for what Chrome APIs exist and how they're organized. **Purpose**: Separates the complex markdown parsing logic from code generation, making it easier to debug parsing issues, see API changes in version control diffs, and reuse the structured data for other purposes (validation, documentation, etc.). Generated by `npm run build:schema` from markdown docs, consumed by `npm run build:mock` to generate TypeScript. Should be committed to version control.
- `src/generated.ts` is a build artifact—do not edit manually. Regenerate with `npm run build:mock` after schema changes.
...
- Never edit `lib/`, `dist/`, or `src/generated.ts` directly—rebuild instead.

## Common Task Playbooks
- **Add/Update Chrome namespace**
  1. `npm run build:docs`
  2. `npm run build:schema`
  3. `npm run build:mock` (generates `src/generated.ts` from updated schema)
  4. Update `src/` helpers if the schema introduces new behavior.
  5. Add/adjust tests under `tests/api/`.
  6. Run `npm test` and `npm run build`.
  7. Document the change in `docs/CHANGELOG.md`.
...
- `npm version <patch|minor|major>` triggers hooks:
  ...
  - `prepublishOnly`: runs `npm run build:mock && npm run lint && npm run format:check && npm run build && npm test`.
